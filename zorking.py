...
def get_list_plants(img_raw) -> List[MatLike]:
...
for plant in range(1, 28):
    ...
    for index, img_raw in enumerate([img_front, img_back]):
        ...
        front_back_list[index] = get_list_plants(img_raw)
    ...
    for i in range(min_length):
        front_count = count_fruit(front_back_list[0][i])
        back_count = count_fruit(front_back_list[1][i])
...
print(sum_all)
print("average yellow: ", sum(yellow) / len(yellow))
print("average purple: ", sum(purple) / len(purple))
print("average red: ", sum(red) / len(red))
